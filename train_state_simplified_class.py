...
hyperparameters = {
    ...
    "dataset_path": os.path.join(
        PROJECT_DIR, "zarr_data", "simplified_short.zarr.zip"
    ),
    # wandb
...
class DiffusionTrainer:
    def __init__(self, config: dict, checkpoint_path: str = None):
        ...
        self.noise_pred_net = ConditionalUnet1D(
            ...
            num_points=self.OBS_DLO_DIM // self.N_DIM,
        ).to(self.DEVICE)
    ...
    def _train_on_batch(self, batch: dict) -> float:
        ...
        idxs = (batch["idx"].to(self.DEVICE).long())
        ...
        # Predict noise
        noise_pred = self.noise_pred_net(noisy_actions, timesteps, global_cond=obs_cond, predict_index=True)

        # Compute loss
        traj_loss = nn.functional.mse_loss(noise_pred["trajectory"], noise)
        # idx does not need action horizon and shoul not be normalized:
        idxs = idxs[:, 0]
        idxs = normalization_pca.denormalize_min_max(
            idxs, self.stats["idx"]["min"], self.stats["idx"]["max"]
        )
        idxs = idxs.long()  

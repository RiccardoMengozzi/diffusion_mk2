...
from diffusion_mk2.model.diffusion.conditional_unet_1d_gauss import ConditionalUnet1D
from diffusion_mk2.dataset.shaping_dataset_gauss import ShapingDataset
...
class DiffusionTrainer:
    def __init__(self, config: dict, checkpoint_path: str = None):
        ...
        self.dataset = ShapingDataset(
            ...
            obs_ee_dim=self.DATASET_OBS_EE_DIM,
            obs_dlo_dim=self.DATASET_OBS_DLO_DIM,
            obs_target_dim=self.DATASET_OBS_TARGET_DIM,
        )
        ...
        # EMA wrapper
        self.ema = EMAModel(
            parameters=self.noise_pred_net.parameters(), power=self.EMA_POWER
        )
    ...
    def save_checkpoint(self, epoch: int, avg_loss: float):
        ...
        checkpoint_path = os.path.join(
            self.checkpoint_dir, f"chkp_{self.run.name}_epoch_{epoch}.pt"
        )
        ...
        wandb.log(
            {"checkpoint_epoch": epoch, "checkpoint_avg_loss": avg_loss},
    ...
    def load_checkpoint(self, checkpoint_path: str):
        ...
        print(
            f"Resuming from epoch {checkpoint['epoch']}, global step {self.global_step}"
        )
    ...
    def _train_on_batch(self, batch: dict) -> float:
        ...
        idxs = (batch["idx"].to(self.DEVICE).float())  # Ensure idxs are float tensor
        ...
        target_state = target_state[:, :, :, :2]  # Keep x, y

        dlo_state = dlo_state.reshape(
        ...
        # Predict noise
        idx_gaussian, noise_pred = self.noise_pred_net(noisy_actions, timesteps, global_cond=obs_cond)


        loss_idx = nn.functional.binary_cross_entropy_with_logits(
            idx_gaussian, idxs
        )

        # Compute L2 loss
        loss_coord = nn.functional.mse_loss(noise_pred, noise)

        loss = loss_idx + loss_coord
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train a diffusion model for push-t state prediction."
    )
    parser.add_argument(
        "--chkp_path",
        type=str,

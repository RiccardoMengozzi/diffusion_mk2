...
class ShapingInference:
    ...
    def __init__(
        ...
        self.most_recent_csR = None  # Most recent csR for denormalization
        ...
        self.action_processor = ActionDataProcessor(
            is_first_idx=False, is_last_gripper=True
        )
    ...
    def _initialize_model(self) -> None:
        ...
        try:
            ...
            self.model = ConditionalUnet1D(
                input_dim=self.action_dim, global_cond_dim=global_cond_dim
            ).to(self.device)
    ...
    def unnormalize_data(
        self, data: Union[torch.Tensor, np.ndarray], stats: Dict[str, np.ndarray]
    ) -> np.ndarray:
        """
    ...
    def run_inference(
        ...
        executable_actions, full_trajectory = self._postprocess_predictions(
            denoised_actions
        )

        return executable_actions, full_trajectory

    def _prepare_observation_input(
        self, observation: Union[np.ndarray, torch.Tensor]
    ) -> torch.Tensor:
        ...
        if observation.shape != expected_shape:
            raise ValueError(
                f"Expected observation shape {expected_shape}, got {observation.shape}"
            )

        return observation

    def _prepare_observation_conditioning(
        ...
        initial_shapes = observation[
            :, initial_shape_range[0] : initial_shape_range[1]
        ].reshape(self.obs_horizon, self.obs_dlo_dim // 3, 3)
        target_shapes = observation[
            :, target_shape_range[0] : target_shape_range[1]
        ].reshape(self.obs_horizon, self.obs_target_dim // 3, 3)

        cs0_list, csR_list = self.dlo_processor.compute_normalize_factors_arrays(
            initial_shapes
        )

        self.most_recent_csR = csR_list[-1]  # Store the most recent csR for denormalization
        ...
        initial_shape_n = initial_shape_n.reshape(-1, self.obs_dlo_dim)
        target_shape_n = target_shape_n.reshape(-1, self.obs_target_dim)
        ee_state_n = ee_state_n.reshape(-1, self.obs_ee_dim)


        obs_cond = np.concatenate([ee_state_n, initial_shape_n, target_shape_n], axis=1).flatten()
        obs_cond = torch.tensor(
            obs_cond, dtype=torch.float32, device=self.device
        ).unsqueeze(0)
    ...
    def _run_denoising_loop(
        ...
        for i, timestep in enumerate(self.noise_scheduler.timesteps):
            ...
            noise_pred = self.model(
                sample=naction, timestep=timestep, global_cond=obs_cond
            )
    ...
    def _postprocess_predictions(
        self, denoised_actions: Tuple[torch.Tensor, list]
    ) -> Tuple[np.ndarray, np.ndarray]:
        ...
        self.action_processor.set_scale_min_max(
            min=self.dataset_stats["action"]["min"],
            max=self.dataset_stats["action"]["max"],
        )

        action_pred = self.action_processor.denormalize(final_action_np, 
                                                        descale=True,
        ...
        all_actions_np = np.array(
            all_actions
        )  # Shape: (num_steps, pred_horizon, action_dim)
        # full_trajectory = self.unnormalize_data(all_actions_np, stats=self.dataset_stats['action'])
        full_trajectory = []
        for action in all_actions_np:
            full_trajectory.append(
                self.action_processor.denormalize(action, 
                                                  descale=True,
                                                  csR=self.most_recent_csR,
                                                  min=self.dataset_stats["action"]["min"],
                                                  max=self.dataset_stats["action"]["max"])
...
if __name__ == "__main__":
    ...
    try:
        model = ShapingInference(
            "/home/mengo/Research/LLM_DOM/diffusion_mk2/weights/chkp_dummy-529qk1bd_epoch_30.pt", verbose=True
        )
        ...
        print("\n--- Silent mode example ---")
        silent_model = ShapingInference(
            "/home/mengo/Research/LLM_DOM/diffusion_mk2/weights/chkp_dummy-529qk1bd_epoch_30.pt", verbose=False
        )

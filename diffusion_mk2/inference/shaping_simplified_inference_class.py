...
class ShapingInference:
    ...
    def _denoise_actions(self, obs_cond: torch.Tensor) -> Tuple[torch.Tensor, list]:
        ...
        for i, timestep in enumerate(self.noise_scheduler.timesteps):
            ...
            noisy_action = self.noise_scheduler.step(
                model_output=noise_pred["trajectory"], 
                timestep=timestep, 
                sample=noisy_action
            ).prev_sample
        ...
        return noise_pred["index_logits"], noisy_action, trajectory
    ...
    def _postprocess_actions(self, final_action: torch.Tensor, trajectory: list) -> Tuple[np.ndarray, np.ndarray]:
        ...
        final_action_pos = final_action[:, :2]
        ...
        for actions in trajectory:
            actions_pos = actions[:, :2]  # Assuming first 2 columns are positions
    ...
    def run_inference(self, observation: Union[np.ndarray, torch.Tensor]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        return idx_logits, executable_actions, full_trajectory
...
if __name__ == "__main__":
    ...
    checkpoint_path = "/home/mengo/Research/LLM_DOM/diffusion_mk2/checkpoints/dummy-bunyvmlb/chkp_dummy-bunyvmlb_epoch_1.pt"
    ...
    # Run inference
    idx_logits, actions, trajectory = model.run_inference(dummy_obs)
    
    print(f"Index logits shape: {idx_logits.shape}")
    print(f"Executable actions shape: {actions.shape}")
    print(f"Full trajectory shape: {trajectory.shape}")
    print("idx_logits:", idx_logits )
    print(f"First action to execute: idx={np.argmax(idx_logits.squeeze().cpu().numpy())}, action={actions[0]}")

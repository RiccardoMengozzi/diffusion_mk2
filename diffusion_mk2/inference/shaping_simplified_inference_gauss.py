...
class ShapingInference:
    ...
    def _denoise_actions(self, obs_cond: torch.Tensor) -> Tuple[torch.Tensor, list]:
        ...
        for i, timestep in enumerate(self.noise_scheduler.timesteps):
            # Predict noise
            idx_logits, noise_pred = self.model(
                sample=noisy_action, 
                timestep=timestep, 
        ...
        return idx_logits, noisy_action, trajectory

    def _postprocess_actions(self, idx_logits: torch.Tensor, final_action: torch.Tensor, trajectory: list) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Apply softmax to idx_logits along the last dimension
        probs = F.softmax(idx_logits, dim=-1)
        print("probs = ", probs)

        # Get the indices of the max probability values
        idxs = probs.argmax(dim=-1).squeeze(0).cpu().numpy()

        final_action_pos = final_action[:, 0:2]
        final_action_theta = final_action[:, 2]
        ...
        for actions in trajectory:
            ...
            actions_pos = actions[:, 0:2]  # Assuming first 2 columns are positions
            actions_theta = actions[:, 2]
    ...
    def run_inference(self, observation: Union[np.ndarray, torch.Tensor]) -> Tuple[np.ndarray, np.ndarray]:
        ...
        with torch.no_grad():
            ...
            idx_logits, final_action, trajectory = self._denoise_actions(obs_cond)

            # Post-process results
            executable_actions, full_trajectory = self._postprocess_actions(idx_logits, final_action, trajectory)
...
if __name__ == "__main__":
    ...
    checkpoint_path = "/home/mengo/Research/LLM_DOM/diffusion_mk2/weights/chkp_resilient-armadillo-50_epoch_7400.pt"

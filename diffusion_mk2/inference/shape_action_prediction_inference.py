...
from diffusion_mk2.model.simple_fc.fc_mul import FCMul, EarlyStopping
...
class ActionFinderGradient:
    def __init__(
        ...
        # MODEL
        self.model = FCMul(
            n_pts=state["num_points"],
            pts_dim=state["dim_points"],
    ...
    def run(self, dlo_0, dlo_1, action_gt=None, idx=None):
        ...
        dlo_0_n, dlo_1_n, action_gt_n = self.sample_obj.normalize(
            dlo_0, dlo_1, action_gt
        )
        ...
        action_init_idx_n = normalization_pca.normalize_min_max(action_init_idx, 0, self.num_nodes - 1)
        action_init_pos_n = normalization_pca.normalize_pca(action_init_pos, cs0, csR, rotation_only=True)
        action_init_theta_n = action_init_theta

        action_init_n = np.array([action_init_idx_n, action_init_pos_n[0], action_init_pos[1], action_init_theta_n])
        ...
        best_action_idx_n = best_action_n[0]
        best_action_pos_n = best_action_n[1:3]
        best_action_theta_n = best_action_n[3]

        best_action_idx = normalization_pca.denormalize_min_max(best_action_idx_n, 0, self.num_nodes - 1)
        best_action_pos = normalization_pca.denormalize_pca(best_action_pos_n, cs0, csR, rotation_only=True)
        best_action_theta = best_action_theta_n  

        best_action = np.array([best_action_idx, best_action_pos[0], best_action_pos[1], best_action_theta])
        ...
        print(pred.shape, pred_init.shape, pred_gt.shape)
    ...
    def find_action(self, dlo_0, dlo_1, action, print_every=10):
        ...
        for step in range(self.num_steps):
            ...
            action_tn = torch.cat([idx.unsqueeze(0)], dim=1)
...
if __name__ == "__main__":
    ...
    # Example paths – update these accordingly
    checkpoint_path = "/home/mengo/Research/LLM_DOM/diffusion_mk2/weights/shape_prediction_final_model.pt"

    # Initialize the class
    af = ActionFinderGradient(
        checkpoint_path=checkpoint_path, device="cpu", verbose=True
    )
